  ...
  schemaComposer.forEach(type => {
    ...
    if ((type instanceof ObjectTypeComposer || type instanceof InterfaceTypeComposer) && type.hasExtension(`childOf`)) {
      ...
      const {
        types,
        mimeTypes
      } = type.getExtension(`childOf`); // TODO: deprecate `many` argument
...
const addInferredChildOfExtension = ({
  ...
  const shouldInfer = typeComposer.getExtension(`infer`); // In Gatsby v3, when `@dontInfer` is set, `@childOf` extension will not be
  ...
  Object.keys(childNodesByType).forEach(typeName => {
    ...
    if (shouldInfer === false) {
      var _childOfExtension$typ2, _childOfExtension;
      ...
      const childOfTypes = ((_childOfExtension$typ2 = (_childOfExtension = childOfExtension) === null || _childOfExtension === void 0 ? void 0 : _childOfExtension.types) !== null && _childOfExtension$typ2 !== void 0 ? _childOfExtension$typ2 : []).concat(parentTypeName).map(name => `"${name}"`).join(`,`);
      report.warn(`Deprecation warning: ` + `In Gatsby v3 fields \`${parentTypeName}.${childField}\` and \`${parentTypeName}.${childrenField}\` ` + `will not be added automatically because ` + `type \`${typeName}\` does not explicitly list type \`${parentTypeName}\` in \`childOf\` extension.\n` + `Add the following type definition to fix this:\n\n` + `  type ${typeName} implements Node @childOf(types: [${childOfTypes}]) {\n` + `    id: ID!\n` + `  }\n\n` + `https://www.gatsbyjs.com/docs/actions/#createTypes`);
