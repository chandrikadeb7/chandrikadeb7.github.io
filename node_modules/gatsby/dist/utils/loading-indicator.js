...
function writeVirtualLoadingIndicatorModule() {
  if (indicatorEnabled === undefined) {
    indicatorEnabled = process.env.GATSBY_EXPERIMENTAL_QUERY_ON_DEMAND && process.env.GATSBY_QUERY_ON_DEMAND_LOADING_INDICATOR === `true` ? `initial` : false;
  }

  (0, _gatsbyWebpackVirtualModules.writeModule)(`$virtual/loading-indicator.js`, `
    export function isLoadingIndicatorEnabled() {
    return ${indicatorEnabled === `initial` ? `\`Cypress\` in window
          ? false
